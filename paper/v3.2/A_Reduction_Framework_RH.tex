...
\ifdefined\RHBuildSupp
  \IfFileExists{xr-hyper.sty}{\usepackage{xr-hyper}}{\usepackage{xr}}
  \let\RHorigbibcite\bibcite
  \renewcommand{\bibcite}[2]{}
  \externaldocument{A_Reduction_Framework_RH_core}
  \let\bibcite\RHorigbibcite
\fi
...
\ifdefined\RHBuildSupp
\title{A Reduction Framework for the Riemann Hypothesis via Global $J$-Contractivity of a Limit-Point Canonical System Derived from $\xi$ (Supplement)}
\fi
...
\ifdefined\RHBuildCore
  \ifdefined\RHBuildSupp
    \errmessage{Define only one build flag: \string\RHBuildCore or \string\RHBuildSupp}
  \fi
\fi
...
\begin{abstract}
\ifdefined\RHBuildSupp
This document is a supplement to \texttt{A\_Reduction\_Framework\_RH\_core}.
It collects auxiliary modules (R2 bookkeeping/normal forms, A2 finite-section Toeplitz certificates, empirical/data snapshots, and local Schur diagnostics) that are not used in the logical implication chain to RH in the core document.
\else
...
the upper half--plane $\UHP$, and $W$ is defined from $f(z)=\xi(\tfrac12+i z)$
...
\]
where $C_{\mathrm{val}}(m):=\frac{m-\ii}{m+\ii}$ (with inverse $C_{\mathrm{val}}^{-1}(w)=\ii\,\frac{1+w}{1-w}$) and $\cdot$ denotes the M\"obius action
of $2\times2$ matrices on $\widehat{\C}$.
Then, as M\"obius maps (i.e. in $\mathrm{PGL}(2,\C)$), $F(\,\cdot\,;z)$ is represented by the explicit matrix
...
\end{pmatrix}.
\]
Represent these Cayley maps in $\mathrm{PGL}(2,\C)$ by
\[
...
\ifdefined\RHBuildCore
The standard Schur step is recalled in the supplementary module R2 as a comparison normal form in $\mathrm{Aut}(\DD)$ and for cocycle bookkeeping,
\else
...
(= a first jet), without any multi-point fitting; see Lemma~\ref{lem:R0_jet_pinning}.
...
\ifdefined\RHBuildCore
The supplementary document separates the auxiliary modules R2, A2, empirical/data snapshots, and local Schur diagnostics for reproducibility and stress-testing.
\else
